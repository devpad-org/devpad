...
func (r *repository) Create(ctx context.Context, ws *Workspace) error {
	...
	result, err := r.db.ExecContext(ctx,
		`INSERT INTO workspaces (user_id, name, description, status, container_id, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?)`,
		ws.UserID, ws.Name, ws.Description, ws.Status, ws.ContainerID, now, now,
	)
	if err != nil {
...
func (r *repository) GetByID(ctx context.Context, id int64) (*Workspace, error) {
	...
	err := r.db.QueryRowContext(ctx,
		`SELECT id, user_id, name, description, status, container_id, created_at, updated_at FROM workspaces WHERE id = ?`,
		id,
	).Scan(&ws.ID, &ws.UserID, &ws.Name, &ws.Description, &ws.Status, &ws.ContainerID, &ws.CreatedAt, &ws.UpdatedAt)
...
func (r *repository) ListByUserID(ctx context.Context, userID int64) ([]*Workspace, error) {
	rows, err := r.db.QueryContext(ctx,
		`SELECT id, user_id, name, description, status, container_id, created_at, updated_at FROM workspaces WHERE user_id = ? ORDER BY created_at DESC`,
		userID,
	)
	if err != nil {
	...
	for rows.Next() {
		ws := &Workspace{}
		if err := rows.Scan(&ws.ID, &ws.UserID, &ws.Name, &ws.Description, &ws.Status, &ws.ContainerID, &ws.CreatedAt, &ws.UpdatedAt); err != nil {
...
func (r *repository) Update(ctx context.Context, ws *Workspace) error {
	...
	_, err := r.db.ExecContext(ctx,
		`UPDATE workspaces SET name = ?, description = ?, status = ?, container_id = ?, updated_at = ? WHERE id = ?`,
		ws.Name, ws.Description, ws.Status, ws.ContainerID, now, ws.ID,
	)
	if err != nil {
